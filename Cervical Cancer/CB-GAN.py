...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=n_channels_base, kernel_size=5, stride=1, padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            # nn.LeakyReLU(1 / 5.5, inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base, out_channels=2 * n_channels_base, kernel_size=5, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.Conv1d(in_channels=n_channels_base*2, out_channels=4 * n_channels_base, kernel_size=4, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 4, out_channels=6 * n_channels_base, kernel_size=3, stride=1,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(in_channels=n_channels_base * 8, out_channels=6 * n_channels_base, kernel_size=5,
                               stride=1, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=n_channels_base * 6, out_channels=4 * n_channels_base, kernel_size=5,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=4 * n_channels_base, out_channels=2*n_channels_base, kernel_size=5,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
...
class Generator(nn.Module):
    def __init__(self):
        ...
        ngf = 4
        self.main = nn.Sequential(
            nn.ConvTranspose1d(opt.latent_dim, ngf * 8, 5, 1, 0),
            nn.BatchNorm1d(ngf * 8, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(ngf * 8, ngf * 4, 5, 2, 0),
            nn.BatchNorm1d(ngf * 4, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf * 4, ngf*2, 4, 2, 0),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 10, 3, 0),
            nn.BatchNorm1d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 7, 3, 0),
            nn.LeakyReLU(0.2, inplace=True),
        )
        ...
        self.conv4 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf * 4, 9, 3, 0),
            nn.BatchNorm1d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 4, 1, 5, 2, 0),
            nn.Sigmoid()
        )
...
if opt.training:
    ...
    for epoch in range(epoch_resume, opt.n_epochs):
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            rec_loss = (mse_loss(fake_decoded, micro_batch))
            #重构mmd误差
            ...
            loss = 8*mmd_loss +e_loss + g_loss-rec_loss
            loss.backward()
...
feature=feature.copy().drop('Biopsy')
...
for j in range(50,51):
    ...
    num_fake_samples = 800
    # Generate a batch of samples
    gen_samples=np.zeros([800, 34])
    ...
    train_f = np.load(f'cancer/syntheticlabel{j}.npy', allow_pickle=False)
    ...
    plt.savefig(f'pictures/real{j}.png')
    plt.close()
    ...
    y_train_lr = X_train_r[:, 0]
    x_test_lr = X_test_r[:, 1:]
    y_test_lr = X_test_r[:, 0]
    ...
    plt.savefig(f'pictures/syn{j}.png')
    plt.close()
    y_pred_rf = cls.predict_proba(X_test_r)[:, 1]
