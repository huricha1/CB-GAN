...
if opt.training:
    ...
    for epoch in range(epoch_resume, opt.n_epochs):
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            rec_loss = (mse_loss(fake_decoded, micro_batch))*2
            ...
            loss = 10*mmd_loss +e_loss + g_loss-rec_loss
            loss.backward()
...
feature=feature.copy().drop('isError')
...
for j in range(50,51):
    ...
    train_f = np.load(f'block/syntheticlabel{j}.npy', allow_pickle=False)
    ...
    y_train_lr = X_train_r[:, 4]

    a = X_test_r[:, :4]
    b = X_test_r[:, 5:]
    x_test_lr = np.concatenate((a, b), axis=1)
    y_test_lr = X_test_r[:, 4]
