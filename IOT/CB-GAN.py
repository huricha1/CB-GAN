...
parser.add_argument("--expPATH", type=str, default=os.path.expanduser('iot/'),
...
trainData=np.delete(trainData,[4,5,6,9,10],axis=1)
testData=np.delete(testData,[4,5,6,9,10],axis=1)
class Dataset:
...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv1d(in_channels=n_channels_base , out_channels=n_channels_base * 2, kernel_size=3, stride=1,
                      padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
            nn.Tanh(),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(in_channels=2 * n_channels_base, out_channels=n_channels_base, kernel_size=4,stride=1, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),

            nn.BatchNorm1d(n_channels_base),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(in_channels=n_channels_base, out_channels=1, kernel_size=3, stride=1,padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.Sigmoid(),
            # nn.PReLU(),
        )
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*8, ngf*4, 3, 1, 0),
            nn.BatchNorm1d(ngf*4, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*4, 1, 2, 1, 0),
            nn.Tanh(),
        )
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=n_channels_base, kernel_size=3, stride=1, padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base, out_channels=n_channels_base*2, kernel_size=3, stride=1, padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base*2, out_channels=n_channels_base * 4, kernel_size=2, stride=1,
                      padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
            nn.Tanh(),
        )
    def forward(self, x):
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 8, 1, 0),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 7, 2, 0),
            nn.BatchNorm1d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf*2, ndf * 4, 7, 2, 0),
            nn.BatchNorm1d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 4, ndf * 8, 6, 2, 0),
            nn.BatchNorm1d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 8, 1, 4, 1, 0),
            nn.Sigmoid()
        )

    def forward(self, x,z):
        out = torch.cat([x,z], dim=1)
        out = self.conv1(out.view(-1, 1, out.shape[1]))
...
if opt.training:
    ...
    for epoch in range(epoch_resume, opt.n_epochs):
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            fake_decoded1 = fake1
            rec_loss = (mse_loss(fake_decoded, micro_batch))*0.05
            ...
            loss = 14*mmd_loss+e_loss + g_loss
            loss.backward()
...
for j in range(50,51):
    ...
    # checkpoint = torch.load('D:/differentially-private-cgan-master/IOT/iot/model_epoch_54.pth')
    # Load models
    generatorModel.load_state_dict(checkpoint['Generator_state_dict'])
    autoencoderModel.load_state_dict(checkpoint['Autoencoder_state_dict'])
    autoencoderDecoder.load_state_dict(checkpoint['Autoencoder_Decoder_state_dict'])
    ...
    for i in range(n_batches):
        ...
        gen_samples_decoded = gen_samples_tensor
    ...
    train_f = np.load(f'iot/syntheticlabel{j}.npy', allow_pickle=False)
    ...
    X_train_r, X_test_r= real_train, real_test
    ...
    y_train_lr = X_train_r[:, 3]

    a = X_test_r[:, :3]
    b = X_test_r[:, 4:]
    x_test_lr = np.concatenate((a, b), axis=1)
    y_test_lr = X_test_r[:, 3]
