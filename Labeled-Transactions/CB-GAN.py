...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 7, 1, 0),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 6, 2, 0),
            nn.BatchNorm1d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf*2, ndf * 4, 6, 2, 0),
...
if opt.training:
    ...
    for epoch in range(epoch_resume, opt.n_epochs):
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            loss = 10*mmd_loss +e_loss + g_loss
            loss.backward()
            ...
            if batches_done % opt.sample_interval == 0:
                print(
                    'TRAIN: [Epoch %d/%d] [Batch %d/%d] Loss_D: %.6f Loss_G: %.6f Loss_E: %.6f Loss_MMD: %.6f Loss_MMD: %.6f'
...
feature=feature.copy().drop('to_scam')
...
for j in range(50,51):
    ...
    train_f = np.load(f'transactions/syntheticlabel{j}.npy', allow_pickle=False)
