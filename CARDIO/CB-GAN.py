...
parser.add_argument("--DATASETPATH", type=str,
                    default=os.path.expanduser('../datasets/cardio_train.csv'),
                    help="Dataset file")
...
parser.add_argument("--latent_dim", type=int, default=64, help="dimensionality of the latent noise space")
...
df1=pd.DataFrame(trainData[['age','height','weight','ap_hi','ap_lo','cardio']])
df=pd.DataFrame(trainData[['gender','active','alco','smoke','gluc','cholesterol']])
df=pd.get_dummies(df,columns=df.columns)
trainData = pd.concat([df,df1],axis=1)

df1 = pd.DataFrame(testData[['age', 'height', 'weight', 'ap_hi', 'ap_lo', 'cardio']])
df = pd.DataFrame(testData[['gender', 'active', 'alco', 'smoke', 'gluc', 'cholesterol']])
df = pd.get_dummies(df, columns=df.columns)

testData = pd.concat([df, df1], axis=1)
...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=n_channels_base, kernel_size=4, stride=1, padding=0, dilation=1,
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.Conv1d(in_channels=n_channels_base*2, out_channels=4 * n_channels_base, kernel_size=3, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 4, out_channels=6 * n_channels_base, kernel_size=3, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 6),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 6, out_channels=n_channels_base * 8, kernel_size=3, stride=1,
                      padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
            nn.Tanh(),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(in_channels=n_channels_base * 8, out_channels=6 * n_channels_base, kernel_size=3,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=n_channels_base * 6, out_channels=4 * n_channels_base, kernel_size=3,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=4 * n_channels_base, out_channels=2*n_channels_base, kernel_size=3,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(in_channels=n_channels_base, out_channels=1, kernel_size=4, stride=1,
                               padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.Sigmoid(),
            # nn.PReLU(),
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            # nn.LeakyReLU(1 / 5.5, inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base, out_channels=2 * n_channels_base, kernel_size=4, stride=1,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 2),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base*4, out_channels=8 * n_channels_base, kernel_size=3, stride=1,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 8, out_channels=n_channels_base * 16, kernel_size=3, stride=2,
                      padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
            nn.Tanh(),
        )
    def forward(self, x):
...
class Generator(nn.Module):
    def __init__(self):
        ...
        ngf = 4
        self.main = nn.Sequential(
            nn.ConvTranspose1d(opt.latent_dim, ngf * 8, 3, 1, 0),
            nn.BatchNorm1d(ngf * 8, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(ngf * 8, ngf * 4, 3, 2, 0),
            nn.BatchNorm1d(ngf * 4, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf * 4, ngf*2, 5, 1, 0),
            nn.BatchNorm1d(ngf*2, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*2, ngf, 5, 1, 0),
            nn.BatchNorm1d(ngf, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf, 1, 4, 2, 0),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 5, 2, 0),
            nn.BatchNorm1d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 5, 4, 0),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(1, ndf, 8, 5, 0),
            nn.BatchNorm1d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 8, 5, 0),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.conv4 = nn.Sequential(
            # state size. (ndf*8) x 4 x 4
            nn.Conv1d(1, ndf * 2, 4, 3, 0),
            nn.BatchNorm1d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 2, 1, 4, 2, 0),
            nn.Sigmoid()
        )
...
if opt.training:
    ...
    for epoch in range(epoch_resume, opt.n_epochs):
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            rec_loss = (mse_loss(fake_decoded, micro_batch))*0.2
            #重构mmd误差
...
feature=feature.copy().drop('cardio')
...
for j in range(50,51):
    ...
    gen_samples[gen_samples[:, 19] >= 0.5, 19] = 1
    gen_samples[gen_samples[:, 19] < 0.5, 19] = 0
    gen_samples=gen_samples[((gen_samples[:,0]==0) & (gen_samples[:,1]==1))|((gen_samples[:,0]==1) & (gen_samples[:,1]==0)),:]
    gen_samples = gen_samples[((gen_samples[:, 2] == 0) & (gen_samples[:, 3] == 1))|((gen_samples[:, 2] == 1) & (gen_samples[:, 3] == 0)), :]
    gen_samples = gen_samples[((gen_samples[:, 4] == 0) & (gen_samples[:, 5] == 1)) | ((gen_samples[:, 4] == 1) & (gen_samples[:, 5] == 0)), :]
    gen_samples = gen_samples[((gen_samples[:, 6] == 0) & (gen_samples[:, 7] == 1)) | ((gen_samples[:, 6] == 1) & (gen_samples[:, 7] == 0)), :]
    gen_samples = gen_samples[((gen_samples[:, 8] == 1) & (gen_samples[:, 9] == 0)&(gen_samples[:, 10] == 0)) | (
                (gen_samples[:, 8] == 0) & (gen_samples[:, 9] == 1)&(gen_samples[:, 10] == 0))|((gen_samples[:, 8] == 0) &(gen_samples[:, 9] == 0)&(gen_samples[:,10] == 1)),:]

    gen_samples = gen_samples[((gen_samples[:, 11] == 1) & (gen_samples[:, 12] == 0) & (gen_samples[:, 13] == 0)) | (
            (gen_samples[:,11] == 0) & (gen_samples[:,12] == 1) & (gen_samples[:,13] == 0))|((gen_samples[:,11] == 0)&(gen_samples[:,12] == 0)&(gen_samples[:,13] == 1)),:]

    if (gen_samples.shape[0] < 56000):
        print("合成数据少于56000")
        continue
    gen_samples = gen_samples[0:56000]
    ...
    # np.savetxt(os.path.join(opt.expPATH, "syntheticlabel1.csv"), gen_samples, delimiter=",")

    train_f = np.load(f'cardio/syntheticlabel{j}.npy', allow_pickle=False)
    ...
    plt.barh(feature1[indices],relevants[indices])
    ...
    x_train_lr=X_train_r[:,:-1]
    y_train_lr=X_train_r[:,-1]
    x_test_lr=X_test_r[:,:-1]
    y_test_lr = X_test_r[:,-1]
    ...
    plt.barh(feature2[indices], relevants[indices])
    ...
    ########## linear_regression ############
    print("linear_regression:")
    x_train_lr = X_train_f[:, :-1]
    y_train_lr = X_train_f[:, -1]
    linear_regressor = LinearRegression()
    linear_regressor.fit(x_train_lr, y_train_lr)
    y_pre = linear_regressor.predict(x_test_lr)
