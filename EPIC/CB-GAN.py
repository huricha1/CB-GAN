...
parser.add_argument("--latent_dim", type=int, default=256, help="dimensionality of the latent noise space")
...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv1d(in_channels=n_channels_base, out_channels=2 * n_channels_base, kernel_size=8, stride=3,
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 4, out_channels=8 * n_channels_base, kernel_size=6, stride=3,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 8, out_channels=n_channels_base * 16, kernel_size=5, stride=1,
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose1d(in_channels=2 * n_channels_base, out_channels=n_channels_base, kernel_size=7,
                               stride=4, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),

            nn.BatchNorm1d(n_channels_base),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(in_channels=n_channels_base, out_channels=1, kernel_size=5, stride=1,
                               padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.Sigmoid(),
            # nn.PReLU(),
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf * 4, ngf*2, 8, 2, 0),
            nn.BatchNorm1d(ngf*2, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*2, ngf, 8, 4, 0),
            nn.BatchNorm1d(ngf, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf, 1, 5, 1, 0),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 9, 2, 0),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 8, 2, 0),
            nn.BatchNorm1d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf*2, ndf * 4, 8, 2, 0),
            nn.BatchNorm1d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 4, ndf * 8, 7, 3, 0),
            nn.BatchNorm1d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 8, ndf * 16, 7, 3, 0),
            nn.BatchNorm1d(ndf * 16),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf * 16, 1, 5, 1, 0),
            nn.Sigmoid()
        )
...
for j in range(50,51):
    ...
    train_f = np.load(f'epic/syntheticlabel{j}.npy', allow_pickle=False)
