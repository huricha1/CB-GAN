...
#chanel增加了
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=n_channels_base, kernel_size=9, stride=2, padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base, out_channels=2 * n_channels_base, kernel_size=8, stride=2, padding=0,
                      dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(2 * n_channels_base),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=2 * n_channels_base, out_channels=4 * n_channels_base, kernel_size=7, stride=3,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(4 * n_channels_base),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=4 * n_channels_base, out_channels=8 * n_channels_base, kernel_size=7, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(in_channels=n_channels_base * 16, out_channels=8 * n_channels_base, kernel_size=10,
                               stride=1, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=n_channels_base * 8, out_channels=4 * n_channels_base, kernel_size=9,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=4 * n_channels_base, out_channels=2*n_channels_base, kernel_size=8,
                               stride=3, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),

            nn.BatchNorm1d(n_channels_base*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=2 * n_channels_base, out_channels=n_channels_base, kernel_size=7,
                               stride=2, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
...
class Generator(nn.Module):
    def __init__(self):
        ...
        ngf = 4
        self.main = nn.Sequential(
            nn.ConvTranspose1d(opt.latent_dim, ngf * 16, 8, 1, 0),
            nn.BatchNorm1d(ngf * 16, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(ngf * 16, ngf * 8, 7, 3, 0),
            nn.BatchNorm1d(ngf * 8, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf * 8, ngf*4, 7, 2, 0),
            nn.BatchNorm1d(ngf*4, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*4, 2*ngf, 6, 2, 0),
            nn.BatchNorm1d(ngf*2, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*2, 1, 3, 1, 0),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 30, 5, 0),
            nn.BatchNorm1d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 25, 6, 0),

            nn.LeakyReLU(0.2, inplace=True),
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(1, ndf, 20, 5, 0),
            nn.BatchNorm1d(ndf),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf*2 , 19, 4, 0),

            nn.LeakyReLU(0.2, inplace=True),
        )
        self.conv4 = nn.Sequential(
            # state size. (ndf*8) x 4 x 4
            nn.Conv1d(1, ndf * 2, 15, 4, 0),
            nn.BatchNorm1d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf*2, 1, 10,4, 0),
            nn.Sigmoid()
        )
...
trainData = np.load(os.path.join(opt.expPATH, "dataTrain.npy"), allow_pickle=False)
testData = np.load(os.path.join(opt.expPATH, "dataTest.npy"), allow_pickle=False)
d=pd.read_csv(os.path.join(opt.expPATH, "df.csv"))
feature=d.columns
feature=feature.copy().drop('y')
...
for j in range(50,51):
    ...
    gen_samples=np.zeros([10000, 179])
    ...
    train_f = np.load(f'uci/syntheticlabel{j}.npy', allow_pickle=False)
