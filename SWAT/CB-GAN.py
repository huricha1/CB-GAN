...
class Autoencoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=n_channels_base, kernel_size=7, stride=1, padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            # nn.LeakyReLU(1 / 5.5, inplace=True),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base, out_channels=2 * n_channels_base, kernel_size=6, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.Conv1d(in_channels=n_channels_base*2, out_channels=4 * n_channels_base, kernel_size=6, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 4, out_channels=8 * n_channels_base, kernel_size=6, stride=2,
                      padding=0, dilation=1,
                      groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(in_channels=n_channels_base * 8, out_channels=n_channels_base * 16, kernel_size=4, stride=1,
                      padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'),
            nn.Tanh(),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(in_channels=n_channels_base * 16, out_channels=8 * n_channels_base, kernel_size=8,
                               stride=1, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=n_channels_base * 8, out_channels=4 * n_channels_base, kernel_size=7,
                               stride=1, padding=0, dilation=1,
                               groups=1, bias=True, padding_mode='zeros'),
            nn.BatchNorm1d(n_channels_base * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(in_channels=4 * n_channels_base, out_channels=2*n_channels_base, kernel_size=6,
...
class Generator(nn.Module):
    def __init__(self):
        ...
        ngf = 8
        self.main = nn.Sequential(
            nn.ConvTranspose1d(opt.latent_dim, ngf * 8, 7, 1, 0),
            nn.BatchNorm1d(ngf * 8, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            # nn.PReLU(),
            nn.ConvTranspose1d(ngf * 8, ngf * 4, 7, 1, 0),
            nn.BatchNorm1d(ngf * 4, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf * 4, ngf*2, 6, 2, 0),
            nn.BatchNorm1d(ngf*2, eps=0.0001, momentum=0.01),
            nn.LeakyReLU(0.2, inplace=True),
            nn.ConvTranspose1d(ngf*2, ngf, 4, 2, 0),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf, 15, 5, 0),
            nn.BatchNorm1d(ndf ),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf, ndf * 2, 9, 4, 0),
            nn.LeakyReLU(0.2, inplace=True),
        )
        ...
        self.conv4 = nn.Sequential(
            ...
            nn.Conv1d(1, ndf * 4, 15, 3, 0),
            nn.BatchNorm1d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv1d(ndf*4, 1, 10, 3, 0),
            nn.Sigmoid()
        )
        ...
        for i_batch, samples in enumerate(dataloader_train):
            ...
            if batches_done % opt.sample_interval == 0:
                print(
                    ...
                    % (epoch + 1, opt.n_epochs, i_batch + 1, len(dataloader_train),d_loss.item(), g_loss.item(),e_loss.item(),mmd_loss.item(),mmd_loss.item()),flush=True)
        # End of epoch
        epoch_end = time.time()
...
for j in range(50,51):
    ...
    # MV101_2
    gen_samples = gen_samples[((gen_samples[:, 0] == 1) & (gen_samples[:, 1] == 0) & (gen_samples[:, 2] == 0)) | ((gen_samples[:, 0] == 0) & (gen_samples[:, 1] == 1) & (gen_samples[:, 2] == 0)) | (
                                          (gen_samples[:, 0] == 0) & (gen_samples[:, 1] == 0) & (gen_samples[:, 2] == 1)), :]
    # P101_2
    gen_samples = gen_samples[((gen_samples[:, 3] == 0) & (gen_samples[:, 4] == 1)) | ((gen_samples[:, 3] == 1) & (gen_samples[:, 4] == 0)), :]
    # P102_2
    gen_samples = gen_samples[((gen_samples[:, 5] == 0) & (gen_samples[:, 6] == 1)) | ((gen_samples[:, 5] == 1) & (gen_samples[:, 6] == 0)), :]
    # MV201_2
    gen_samples = gen_samples[((gen_samples[:, 7] == 1) & (gen_samples[:, 8] == 0) & (gen_samples[:, 9] == 0)) | ((gen_samples[:, 7] == 0) & (gen_samples[:, 8] == 1) & (gen_samples[:, 9] == 0)) | (
                                          (gen_samples[:, 7] == 0) & (gen_samples[:, 8] == 0) & (gen_samples[:, 9] == 1)), :]
    # P206_2
    ...
    train_f = np.load(f'swat/syntheticlabel{j}.npy', allow_pickle=False)
